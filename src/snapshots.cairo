...
mod tests {
    ...
    fn test_get_age() {
        let person = Person { 
         ...
         assert!(age == 30);
        // assert!(person.age == 30); // error: cannot access this becouse ownership is transferred
    }

    fn get_age_return_ownwership(person: Person) -> (u32, Person) {
        (person.age, person)        
    }
    ...
    fn test_get_age_return_ownership() {
        let person = Person { 
         ...
         let (age, person) = get_age_return_ownwership(person);
         assert!(age == 30);
